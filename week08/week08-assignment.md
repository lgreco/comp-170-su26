...
### July 8 — Designing a GPA Tracker, and Catching Bad Input ([notes](./2026-07-08-COMP170.md))

We practiced turning a loose request — "track a student's running GPA, no course database needed" — into working code, landing on a `enter_grade` method ([`gpa.py`](./gpa.py)) that converts a letter grade to a number (A = 4.0, B = 3.0, C = 2.0, D = 1.0, F = 0) and returns the new GPA by averaging it with the old one. Along the way we stressed validating assumptions like the grade scale with the client up front, rather than discovering a mistake partway through a project. We then found two bugs by hand-testing: a lowercase letter fell through every check and silently scored as zero, and a mistyped letter did the same. We fixed the lowercase case by converting input to uppercase with `.upper()`, and discussed `raise`ing a `ValueError` when the letter isn't A, B, C, D, or F, or when a starting GPA is negative — deliberately crashing the program rather than silently corrupting the GPA. This is exactly the pattern Problems 1 and 2 below ask you to apply to a bank account. We also saw Python's `in` operator as a compact way to check membership in a set of acceptable grades (`letter not in 'ABCDF'`), and touched on keeping lines under about 80 characters for readability, a convention with roots in old 80-column punch cards.
