...
**The term has ended.** The weekly sequence runs through `week11/`; `week99/` holds a post-course retrospective (`course-review.md`) and the final assignment (`last_assignment.md`) rather than a numbered week.
...
| `week11/` | `2026-07-27-COMP170.md`, `2026-07-28-COMP170.md`, `2026-07-29-COMP170.md`, `bad_logic_example.py`, `file_processing.py`, `file_processing_dict.py`, `reduced.py`, `tale_of_two_cities.txt` | The final week of the course. Reading and analyzing a real text file (the opening of *A Tale of Two Cities*) word by word; counting word frequencies by hand before in code; the two-synchronized-lists approach ("array synchronization") built first and shown to be fragile (`file_processing.py`); `bad_logic_example.py` is a broken first draft kept on purpose, an `IndexError`-crashing example of a loop's stopping condition being backwards; debugging an off-by-one bug and an infinite loop from a wrong end-of-file check; normalizing case before counting; replacing the synchronized lists with a single dictionary (`file_processing_dict.py`); refactoring a tangled block into two named functions; reading a book directly from a URL with `urllib.request` instead of from a local file. No `week11-assignment.md` — the course ended before a formal assignment was posted for this week. |
| `week99/` | `course-review.md`, `last_assignment.md` | Written after the course ended; not part of the weekly sequence. `course-review.md` is a student-facing retrospective tracing the course's progression week by week, with particular attention to how "Leo's four pillars" (strings, if statements, loops, arrays) were introduced and recombined, plus a section on topics the eleven-week course had no time for (OOP, additional data structures beyond lists/dictionaries, recursion beyond factorial, algorithmic complexity, git, regex, real APIs) as self-study suggestions. `last_assignment.md` is the course's final assignment: a minimal-import URL-reading scaffold (`urllib.request` only), a regex-free punctuation-stripping tutorial, a dictionary tutorial based on Lubanovic, a word-frequency problem (10 most- and least-frequent five-letter words from a Project Gutenberg book), and a 300-word self-graded course reflection (`reflection.txt`) covering attendance, participation, and code quality against posted solutions. |
...
| *Introducing Python, 3rd Ed.* — Ch. 9: Dictionaries and Sets | Textbook (O'Reilly) | https://learning.oreilly.com/library/view/introducing-python-3rd/9781098174392/ch09.html | last assignment (week99) | Creating and using dictionaries (key/value pairs, `in`, `.items()`); referenced for the word-frequency-by-URL problem |
